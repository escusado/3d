	...
	parse: function ( json ) {
		...
		for ( var meshId in meshes ) {
			...
			for ( var i = 0; i < primitives.length; i ++ ) {
				...
				geometries.material = primitive.material;
			...
			if ( node.meshes !== undefined ) {
				...
				for ( var i = 0; i < node.meshes.length; i ++ ) {
					...
					for ( var j = 0; j < array.length; j ++ ) {
						var mat = materials[geometries.material];
						var textureImage = json.images[json.textures[mat.values.diffuse].source];
						var image = new Image();

						image.src = textureImage.uri;
						var matTex = new THREE.Texture(image);
						...
						group.add( new THREE.Mesh( array[ j ], material ) );
					}
